...
# ===============================================
# Domain lifecycle management (design spec 8.2.1)
# ===============================================

def _get_core_soul_path() -> Path:
    """Core soul.md template path."""
    builder_path = os.environ.get("BUILDER_PATH", "")
    if builder_path:
        core = Path(builder_path).parent.parent / "config" / "openclaw" / "soul.md"
        if core.exists():
            return core
    return Path(__file__).resolve().parent.parent.parent.parent / "config" / "openclaw" / "soul.md"


def _get_workspace_soul_paths() -> list[Path]:
    """OpenClaw workspace SOUL.md path list."""
    return [
...
def _get_env_path() -> Path:
    ...
    builder_path = os.environ.get("BUILDER_PATH", "")
    if builder_path:
        env = Path(builder_path).parent.parent / ".env"
        if env.exists():
            return env
...
@mcp.tool()
def switch_domain(domain_name: str) -> str:
    """Switches to domain mode. Copies the domain's soul.md to the OpenClaw workspace.
    Enter "core" to return to Core mode (Domain Builder).

    A session /reset is required in OpenClaw after switching.

    Args:
        domain_name: Domain name (e.g., "my-domain") or "core"
    """
    import shutil

    if domain_name == "core":
        source = _get_core_soul_path()
        if not source.exists():
            return f"Cannot find Core soul.md: {source}"
        label = "Core (Domain Builder)"
    else:
        domain_dir = _domain_dir(domain_name)
        source = domain_dir / "soul.md"
        if not source.exists():
            return (f"Cannot find domain soul.md: {source}\n"
                    f"Run Phase 4 (generate_soul) first.")
        label = domain_name

    copied = 0
    for rp in _get_workspace_soul_paths():
        try:
            rp.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(str(source), str(rp))
            copied += 1
        except Exception as e:
            pass

    if copied == 0:
        return "Failed to copy SOUL.md. Check workspace paths."

    return (
        f"**{label}** mode switch complete.\n\n"
        f"Enter `/reset` in Telegram to apply."
    )


@mcp.tool()
def backup_domain(domain_name: str) -> str:
    """Backs up the entire build result of a domain.
    Saves domains/{domain}/ files + Qdrant indexing data + .env settings to backup folder.
    ...
    if not domain_dir.exists():
        return f"Cannot find domain directory: {domain_dir}"

    # Backup directory
    backup_base = domain_dir.parent.parent / "backups"
    backup_dir = backup_base / f"{domain_name}_{timestamp}"
    backup_dir.mkdir(parents=True, exist_ok=True)

    report = []

    # 1. Copy domain files
    domain_backup = backup_dir / "domain"
    shutil.copytree(str(domain_dir), str(domain_backup))
    file_count = sum(1 for _ in domain_backup.rglob("*") if _.is_file())
    report.append(f"- Domain files: {file_count} files -> `{domain_backup}`")

    # 2. Copy workspace SOUL.md
    for rp in _get_workspace_soul_paths():
        if rp.exists():
            shutil.copy2(str(rp), str(backup_dir / "SOUL.md"))
            report.append(f"- SOUL.md backup complete")
            break

    # 3. Copy .env
    env_path = _get_env_path()
    if env_path.exists():
        shutil.copy2(str(env_path), str(backup_dir / ".env"))
        report.append(f"- .env backup complete")
    ...
    try:
        ...
        # Create snapshot
        resp = httpx.post(
            f"{qdrant_url}/collections/documents/snapshots",
        ...
        # Download snapshot
        dl_resp = httpx.get(
            f"{qdrant_url}/collections/documents/snapshots/{snapshot_name}",
            headers=headers, timeout=120.0
        )
        dl_resp.raise_for_status()
        snapshot_path = backup_dir / f"qdrant_{snapshot_name}"
        snapshot_path.write_bytes(dl_resp.content)
        report.append(f"- Qdrant snapshot: `{snapshot_name}` ({len(dl_resp.content) // 1024}KB)")
    except Exception as e:
        report.append(f"- Qdrant snapshot failed: {e}")

    return (
        f"## Domain Backup Complete\n\n"
        f"**Domain:** {domain_name}\n"
...
@mcp.tool()
def reset_to_core() -> str:
    """Resets to Core mode (Domain Builder).
    Restores workspace SOUL.md to Core template,
    and clears Qdrant/Redis to prepare for new domain build.

    **Warning: Indexing data will be deleted. Run backup_domain first.**
    """
    import shutil

    report = []

    # 1. SOUL.md -> restore Core template
    core_soul = _get_core_soul_path()
    if core_soul.exists():
        copied = 0
        for rp in _get_workspace_soul_paths():
            try:
                rp.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(str(core_soul), str(rp))
                copied += 1
            except Exception:
                pass
        report.append(f"- SOUL.md: Core template restored ({copied} paths)")
    else:
        report.append(f"- SOUL.md: Core template not found ({core_soul})")

    # 2. Delete all Qdrant points
    qdrant_url = os.environ.get("QDRANT_URL", "http://localhost:6333")
    qdrant_api_key = os.environ.get("QDRANT_API_KEY", "nexus-qdrant-change-me")
    try:
        import httpx
        resp = httpx.post(
            ...
            json={"filter": {}}, timeout=30.0
        )
        resp.raise_for_status()
        report.append("- Qdrant: All points deleted")
    except Exception as e:
        report.append(f"- Qdrant reset failed: {e}")

    # 3. Delete Redis queues
    redis_url = os.environ.get("REDIS_URL", "redis://localhost:6379")
    try:
        import redis as redis_lib
        r = redis_lib.from_url(redis_url, decode_responses=True)
        for q in ["nexus:indexing:queue", "nexus:indexing:retry", "nexus:indexing:dead_letter"]:
            r.delete(q)
        report.append("- Redis: All indexing queues deleted")
    except Exception as e:
        report.append(f"- Redis reset failed: {e}")

    # 4. Reset .env DOCS_PATH to default
    env_path = _get_env_path()
    if env_path.exists():
        try:
            import re
            content = env_path.read_text(encoding="utf-8")
            content = re.sub(r"^DOCS_PATH=.*$", "DOCS_PATH=./test-docs", content, flags=re.MULTILINE)
            env_path.write_text(content, encoding="utf-8")
            report.append("- .env: DOCS_PATH reset to default")
        except Exception as e:
            report.append(f"- .env reset failed: {e}")

    report.append("")
    report.append("**Next steps:**")
    report.append("1. Enter `/reset` in Telegram (apply Core mode)")
    report.append("2. Run `docker compose up -d indexing-worker` (update Worker volume mount)")
    report.append("3. Ready to start new domain build")

    return (
        "## Core Mode Reset Complete\n\n"
        + "\n".join(report)
    )
